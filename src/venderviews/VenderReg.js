...
function VenderReg()
{
    ...
    const handleRegisterButton = ()=>{
        ...
        axios.post("https://server-app-xite.onrender.com/vender/register",obj)
        .then((res)=>{
            alert(res.data);
            fetchVendorList();
        })
    ...
    const handleSubmit = async(evt)=>{
        ...
        try{
            const response = await fetch(
                "https://server-app-xite.onrender.com/vender/savevenderimage",
                {
                    method:"POST",
                    body: formData,
                }
            ...
            if(response.ok){
                ...
                setVPicName(data.filename); // important
            } else {
            ...
            <center>
                <div className="vendBox_guru">
                    ...
                    <div className="vendRow_guru">
                        ...
                        {image.preview && (
                            <img src={image.preview} className="vendPreview_guru" height={100} width={100}/>
                        )}
                        <span className="vendError_guru">{errors.vpicname}</span>
                        <button onClick={handleSubmit} className="vendBtn_guru vendBtnDanger_guru">
                            UPLOAD
                        </button>
                    </div>

                    <div className="vendActions_guru">
                        <button onClick={handleRegisterButton} className="vendBtn_guru vendBtnPrimary_guru">
                            REGISTER
                        </button>
                    </div>
                </div>
